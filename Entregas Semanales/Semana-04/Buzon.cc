...
int Buzon::Enviar(int cOO, int cHH, long tipo){
    int resultado;
    a.tipo = tipo;
    a.cO = cOO;
    a.cH = cHH;
    resultado = msgsnd(this->id, (const void *) &a, sizeof(a), IPC_NOWAIT);
    return resultado;
}
int Buzon::Recibir(int cOR, int cHR, long tipo){
    int resultado = 1;
    a.tipo = tipo;
    a.cO = cOR;
    a.cH = cHR;
    resultado = msgrcv(this->id, &a, sizeof(a), tipo, IPC_NOWAIT);
    return resultado;
}
