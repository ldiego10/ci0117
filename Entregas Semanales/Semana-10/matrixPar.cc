...
#pragma omp parallel shared(r) private(x,y,z) 
   {
...
//El omp schedule (dynamic) con matriz de 200*200 dura aproximadamente 0.55ms mientras que las version pthread dura mas y serial duran mucho mas.
//La version schedule(runtime) es aun mas veloz con matriz 200 * 200, dura aproximadamente 0.034ms.
   double time = omp_get_wtime() - start_time;
     printf( "Version OMP: Dura %lf ms\n", time );
    for(int i = 0; i < tamano; i++)
        delete[] r.matA[i];
    delete[] r.matA;
